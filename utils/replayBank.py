...
class ReplayBank:
    ...
    def store_samples(self, dataset:DummyDataset, model):
        ...
        if self._fixed_memory: #fix的话，意思是每类fix
        ...
        for class_idx in class_range:
            ...
            self.select_sample_indices(self._sampling_method, idx_vectors, per_class)
            self._logger.info("New Class {} instance will be stored: {} => {}".format(class_idx, len(idx_targets), len(selected_idx)))
            #也就是说，每个类别本来有多少张train sample，现在变成多少
            # to reduce calculation when applying replayBank (expecially for some methods do not apply nme)
            if self._apply_nme:
                # 计算类中心
    ...
    def store_samples_more(self, dataset:DummyDataset):
        #和下面的都是一样的，因为store_samples函数本来就支持random。
        """dataset 's transform should be in test mode!"""
        class_range = np.unique(dataset.targets)
        assert min(class_range)+1 > len(self._class_sampler_info), "Store_samples's dataset should not overlap with buffer"
        if self._fixed_memory: #fix的话，意思是每类fix
        ...
        for class_idx in class_range:
            ...
            cur_class_sample_num = len(class_data_idx)
            
            selected_idx =self.select_sample_indices_more(cur_class_sample_num, per_class)
            ...
            self._class_sampler_info.append(len(selected_idx))#self._class_sampler_info中存储了每个类实际保存的样本数[200,200,200,200]之类的
    ...
    def get_memory_dataset(self):
        replay_data, replay_targets = self.get_memory()
        return DummyDataset(replay_data, replay_targets, None, False)
    ...
    def select_sample_indices_more(self, cur_class_sample_num, m): #
        idxes = np.arange(cur_class_sample_num)
        np.random.shuffle(idxes)# 防止类别数过少的情况
        if cur_class_sample_num > m:
            store_sample_size = m
        else:
            self._logger.info('The whole class samples are less than the allocated memory size!')
            store_sample_size = cur_class_sample_num
        self._logger.info('currently only support random mode')
    ...
    def _extract_vectors_more(self, model, loader, ret_data=False, ret_add_info=False):
        model.eval()
        vectors = []
        logits = []
        data = []
        ...
        with torch.no_grad():
            for _add_info, _inputs, _targets in loader:
                output_features, _ = model.forward_features(_inputs.cuda())
                out, output_features = model()
                vectors.append(output_features['features'])
                logits.append(out)
                if ret_data:
                    data.append(_inputs)
