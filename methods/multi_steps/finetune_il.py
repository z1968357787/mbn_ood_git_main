...
class Finetune_IL(BaseLearner):
    ...
    def eval_task(self):        
        ...
        if self._eval_metric == 'acc':
            cnn_total, cnn_task = accuracy(cnn_pred.T, y_true, self._total_classes, self._increment_steps) ##cnn_total: cil total acc of all tasks, cnn_task: cil acc of all tasks
    ...
    def save_checkpoint(self, filename, model, task_id): #filename以.pkl结尾
        ...
        torch.save(save_dict, save_path) #torch.save保存字典形式
    ...
    def _get_scheduler(self, optimizer, config, is_init:bool):
        ...
        if is_init:
            if config.scheduler == 'multi_step':
                ...
                scheduler = None
            elif config.scheduler == "warmup_plus_cosine_anneal":
                # pass
                if config.non_linear_after_scheduler == 'cosine':
                    scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer, config.epochs)
                    #
                elif config.non_linear_after_scheduler == 'step_decay':
                    milestones = [int(0.5 * self._epochs), int(0.75 * config.epochs)]
                    scheduler = optim.lr_scheduler.CosineAnnealingLR.MultiStepLR(optimizer, gamma=self.lr_decay_gamma, milestones=milestones)
                else:
                    raise NotImplementedError()

                scheduler = GradualWarmupScheduler(optimizer, multiplier=10.0, total_epoch = 10, after_scheduler = scheduler)

            elif config.lr_scheduler == 'step_decay':
                milestones = [int(0.5 * self._.epochs), int(0.75 * self._epochs)]
                scheduler = optim.lr_scheduler.MultiStepLR(optimizer, gamma = self._init_lrate_decay, milestones=milestones)
