...
def get_backbone(logger, backbone_type, pretrained=False, pretrain_path=None, normed=False) -> nn.Module:
    ...
    elif name == "resnet18_cifar_mbn":
        net = resnet18_mbn(normed=normed)
...
def get_backbone_abl_shared_bn(logger, backbone_type, pretrained=False, pretrain_path=None, normed=False) -> nn.Module:
    #此函数仅在mbn_ori_refer_to_adapter_abl_tsb里面使用，用于创建共享bn的特征提取器。该特征提取器由：第一个任务训练后的bn来获得
    name = backbone_type.lower()
    net = None
    if name == 'resnet32':
        net = resnet32()
...
class IncrementalNet(nn.Module):
    ...
    def __init__(self, logger, backbone_type, pretrained, pretrain_path=None, layer_names: Iterable[str]=[], MLP_projector=False):
        ...
        '''
        self._logger = logger 
        #继承IncrementalNet的话，那就需要把backbone_type写到get_backbone函数里面
