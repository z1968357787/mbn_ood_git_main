import torch
import copy
from torch import nn
from torch.utils.data import DataLoader

from backbone.inc_net import get_backbone

## 这个函数，是没有魔改的形式
# 也就是说：训练的时候，添加特征提取器，但是只把bn层设为是可训练的。
# 等价于：每个任务到来的时候，添加bn层。（因为每个任务到来的时候，其他的模块是fix住的）
# 对应的方法名称是：mbn_ori_refer_to_adapter


class mbn_net_ori_refer_adapter(nn.Module):
    def __init__(self, logger, backbone_type, pretrained, pretrain_path=None):
        super(mbn_net_ori_refer_adapter,self).__init__()
        self._logger = logger
        self.backbone_type = backbone_type
        self.feature_extractors = nn.ModuleList()
        
        self.pretrained = pretrained
        self.pretrain_path = pretrain_path
        self.out_dim = None
        self.seperate_fcs = nn.ModuleList()
        
        self.seperate_fc = None
        self.task_sizes = []
        
    def train_bn_mode(self):
        self._training_mode = 'train_bn'
        self._logger.info("Training mode 'train_bn' is set !")
        
    # def freeze_previous_FE_HEAD(self): #only freeze fe and set eval mode
    #     #旧的分类头其实不用冻结，因为不使用它就可以了，就不会有梯度以及更新
    #     if self._cur_task > 0:
    #         for i in range(self._cur_task):
    #             for p in self.feature_extractors[i].parameters(): #冻结过去的特征提取器的参数
    #                 p.requires_grad = False
    #             for p in self.seperate_fcs[i].parameters(): #冻结过去的分类头的参数
    #                 p.requires_grad = False
                    
    #             self.feature_extractors[i].eval()
    #             self._logger.info('Freezing task extractor{} ! '.format(i))
    
    def freeze_previous_FE(self): #only freeze fe and set eval mode
        if self._cur_task > 0:
            for i in range(self._cur_task):
                for p in self.feature_extractors[i].parameters(): #冻结过去的特征提取器的参数
                    p.requires_grad = False

                self.feature_extractors[i].eval()
                self._logger.info('Freezing task extractor{} ! '.format(i))
        
    
    def test_mode(self):
        self._training_mode = 'test_mode'
        self._logger.info("Training mode 'test_mode' is set !")
        
        
    def freeze_bn(self, mode):
        if mode == "all":
            for i in range(self._cur_task + 1):
                cur_fe = self.feature_extractors[i]
                for name, param in cur_fe.named_parameters():
                        param.requires_grad = False
                cur_fe.eval()
                
    def freeze_bn_head(self, mode): #test_metric这个函数用的，其他的基本都不用
        if mode == "all":
            for i in range(self._cur_task + 1):
                cur_fe = self.feature_extractors[i]
                cur_fc = self.seperate_fcs[i]
                for name, param in cur_fe.named_parameters():
                    param.requires_grad = False
                for name, param in cur_fc.named_parameters():
                    param.requires_grad = False
                cur_fe.eval()
                cur_fc.eval()
                
                
    def new_bn_train(self): #set only the new bn train, rest eval
        self.eval() #set all to be eval mode (stop calculate bn statistics)
        self.feature_extractors[-1].train() #only the current one to be train mode (calculate bn statistics)
        self.aux_fc.train()
        
    def extract_features(self, x): #need to modify
        features = self.feature_extractors[-1](x)

        return features

    def forward(self, x):  #if the first stage, only consider forward the model in the current feature extractor
        features = self.feature_extractors[-1](x)

        aux_logits=self.aux_fc(features)  #in the first stage, use aux_fc
        return aux_logits, {"features": features} #必须统一通过这种形式包裹

    def forward_all_model(self, x):  #forward test是：使用所有的任务对应的模型进行前向
        self.forward_batch_size = x.shape[0]
        
        task_feature_list = []
        task_output_list = []
        for i in range(len(self.task_sizes)): # features: [b * number  of adapters, feature_dim]
            self.feature_extractors[i].eval()
            self.seperate_fcs[i].eval()
            task_feature = self.feature_extractors[i](x)
            task_feature_list.append(task_feature) # features: [b, feature_dim]
            task_output_list.append(self.seperate_fcs[i](task_feature))
        
        features = torch.cat(task_feature_list, dim=1) # features: [b, feature_dim * number of adapters]
        aux_logits = torch.cat(task_output_list, dim=1) # seperate logits: [b, known_class_num]
        return aux_logits, {"features": features} #必须统一通过这种形式包裹


    # def forward_test(self, x): 
    #     #如果是测试的时候，需要在所有任务对应的模型的基础上进行前向
    #     features = self.feature_extractors[-1](x)

    #     aux_logits=self.seperate_fcs[-1](features)  #seperate_fc其实就是fc，只不过是多了一个logit的fc
    #     return aux_logits, {"features": features} #必须统一通过这种形式包裹


    def update_fc(self, nb_classes): 
        #not only update the fc, also update the feature extractor(in fact, only train bn)
        new_task_size = nb_classes - sum(self.task_sizes)
        self.task_sizes.append(new_task_size)

        ft = get_backbone(self._logger, self.backbone_type, pretrained=self.pretrained, pretrain_path=self.pretrain_path)
        # ft = get_backbone(self._logger, self.backbone_type, pretrained = True, pretrain_path=self.pretrain_path) #预训练模型是成功载入的。
        
        if 'resnet' in self.backbone_type:
            feature_dim = ft.fc.in_features
            self._logger.info("feature dim is {}".format(feature_dim))
            ft.fc = nn.Identity()
        elif 'efficientnet' in self.backbone_type:
            feature_dim = ft.classifier[1].in_features
            ft.classifier = nn.Dropout(p=0.4, inplace=True)
        elif 'mobilenet' in self.backbone_type:
            feature_dim = ft.classifier[-1].in_features
            ft.classifier = nn.Dropout(p=0.2, inplace=False)
        else:
            raise ValueError('{} did not support yet!'.format(self.backbone_type))

        if len(self.feature_extractors)==0:
            self.feature_extractors.append(ft)
            self._feature_dim = feature_dim
        else:
            self.feature_extractors.append(ft)
        self.aux_fc =self.generate_fc(self._feature_dim, new_task_size + 1) #aux_fc used in the first stage, 
        # in the second stage abandon aux_fc, use finetune loader to retrain "seperate_fc"
         
        self.seperate_fc=self.generate_fc(self._feature_dim, new_task_size + 1)
        self.seperate_fcs.append(self.seperate_fc)    

    def generate_fc(self, in_dim, out_dim):
        # fc = SimpleLinear(in_dim, out_dim)
        fc = nn.Linear(in_dim, out_dim)
        return fc

    def copy(self):
        return copy.deepcopy(self)

    def freeze_feature_extractor(self):
        for param in self.feature_extractor.parameters():
            param.requires_grad = False
        self.feature_extractor.eval()

    def reset_fc_parameters(self):
        nn.init.kaiming_uniform_(self.fc.weight, nonlinearity='linear')
        nn.init.constant_(self.fc.bias, 0)