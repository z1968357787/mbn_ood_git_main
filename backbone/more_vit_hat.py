...
class Adapter(nn.Module):
    ...
    def append_embedddings(self):
        self.ec1.append(nn.Parameter(torch.randn(1, self.out_dim).cuda()))
        self.ec2.append(nn.Parameter(torch.randn(1, self.in_dim).cuda()))
...
class MyVisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        self.patch_embed = embed_layer(
            img_size=img_size, patch_size=patch_size, in_chans=in_chans, embed_dim=embed_dim, norm_layer=self.norm_layer)
...
@torch.no_grad()
def _load_weights(model: MyVisionTransformer, checkpoint_path: str, prefix: str = ''):
